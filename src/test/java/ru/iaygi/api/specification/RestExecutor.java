...
public class RestExecutor {
    ...
    public <T> List<T> getResponseAsList(String path, Class<T> tClass) {
        return getResponseAsJson().getList(path, tClass);
    }
    ...
    public RestExecutor followRedirects(boolean value) {
        restAssuredConfig = restAssuredConfig.redirect(
                RedirectConfig.redirectConfig().followRedirects(value));

        return this;
    }

    public RestExecutor appendDefaultCharset(boolean value) {
        restAssuredConfig = restAssuredConfig.encoderConfig(
                EncoderConfig.encoderConfig().appendDefaultContentCharsetToContentTypeIfUndefined(value));
    ...
    public RestExecutor setSendFile(File file, String contentType) {
        MultiPartSpecBuilder multipart = new MultiPartSpecBuilder(file)
    ...
    private Response sendRequest(Method requestType, String uri) {
        log.debug("Send Request to URI: " + uri);
        if (uriDefined) {
